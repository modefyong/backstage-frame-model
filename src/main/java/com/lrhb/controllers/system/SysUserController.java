...
public class SysUserController {
    ...
    public IResult getUserList(String page, String limit, String loginName, String realName, String status){
        ...
        return new PageResultBean<Collection<User>>(sysUserServiceImpl.getAll(page,limit,loginName,realName,status),sysUserServiceImpl.countGetAll(loginName,realName,status));
    }
     ...
     * 根据部门名称获取用户列表
     * */
    @RequestMapping(value = "/getDepUser.do",method = RequestMethod.POST)
    @ResponseBody
    public IResult getDepUser(String department){
        //返回json至前端的均返回ResultBean或者PageResultBean
        return new ResultBean<Collection<User>>(sysUserServiceImpl.getUsersByDepartment(department));
    }
