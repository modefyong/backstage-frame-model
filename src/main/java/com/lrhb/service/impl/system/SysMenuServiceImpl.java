...
import com.lrhb.dataaccess.dao.system.SysMenuRoleMapper;
import com.lrhb.dataaccess.dao.system.SysMenuMapper;
...
import com.lrhb.security.ShiroConfigService;
...
import com.lrhb.utils.ShiroFilterSortUtil;
...
public class SysMenuServiceImpl implements SysMenuService {
    ...
    @Autowired
    SysMenuRoleMapper sysMenuRoleTestMapper;
    @Autowired
    SysUserRoleMapper sysUserRoleTestMapper;
    @Autowired
    SysMenuMapper sysMenuTestMapper;
    @Autowired
    SysRoleMapper sysRoleTestMapper;
    @Autowired
    SysAccessPermissionMapper sysAccessPermissionTestMapper;
     ...
     * @param User 用户
     */
    public List<MenuTreeResponse> getMenuByRoles(User user) {
        //拿到角色id列表
        List<SysRole> roles = sysUserRoleTestMapper.getRolesByUserId(user.getId());
    ...
    public String addMenu(String menuName, String menuUrl, String menuType, String parentMenuId, String requestUrl, String sort) {
        ...
        //菜单名称不能重复
        SysMenu menuTest = sysMenuTestMapper.getByMenuName(menuName);
    ...
    public Boolean deleteMenu(String menuId) {
        ...
        SysMenu menu = sysMenuTestMapper.selectByPrimaryKey(menuId);
        ...
        //删除访问权限表对应记录
        SysAccessPermission sysAccessPermissionTest = sysAccessPermissionTestMapper.selectByUrl(menu.getRequestUrl());
    ...
    public SysMenuExtend getById(String id) {
        SysMenuExtend menu = sysMenuTestMapper.getById(id);
...
/**************************私有方法区*****************************************/
    ...
    private String generateMenuCode(String menuType, String parentMenuId) {
        ...
        if (Constants.MenuType.FIRST_CLASS.value.equals(menuType)) {//一级菜单
            ...
            SysMenu parentMenu = sysMenuTestMapper.selectByPrimaryKey(parentMenuId);
            if (StringUtils.isBlank(maxCode)) {
    ...
    private void updateMenuRole(String menuId, List<String> idList) {
        ...
        for (String roleId : idList) {
            SysMenuRole menuRoleTest = new SysMenuRole();
     ...
     */
    private int insertOrUpdateMenuRole(SysMenuRole sysMenuRoleTest) {
        ...
        SysMenuRole oldMenu = sysMenuRoleTestMapper.selectByMenuIdAndRoleId(sysMenuRoleTest.getMenuId(), sysMenuRoleTest.getRoleId());
        if (null == oldMenu) {
    ...
    private void updateAccessPrivilege(String requestUrl, List<String> roleIdList) {
        ...
        if (!(roleIdList == null || roleIdList.isEmpty())) {
            ...
            //更新
            SysAccessPermission sysAccessPermissionTest = sysAccessPermissionTestMapper.selectByUrl(requestUrl);
            ...
            //新增
            sysAccessPermissionTest = new SysAccessPermission();
