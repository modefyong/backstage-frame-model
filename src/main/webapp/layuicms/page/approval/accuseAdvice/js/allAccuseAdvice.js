layui.config({
    ...
    function defineTable() {
        ...
        tableIns = table.render({
        	elem: '#accuseAdviceList-data'
           ,toolbar: '#toolbarDemo'
           ,height: 600
           ,url: $tool.getContext() + 'approval/getAccuseAdviceList.do' //数据接口
           ,method: 'post'
           ,where: {
           ...
           ,cols: [[ //表头
        	   {type: 'numbers', title: '序号', fixed: 'left'}
          	  ,{field: 'userId', title: '申请人', width: 120}
          	  ,{field: 'type', title: '类型', width: 120, templet: "#typeTemplate"}
          	  ,{field: 'title', title: '简要', width: 240}
          	  ...
          	  ,{field: 'createTime', title: '发起时间', width: 180, templet: "#timeTemplate"}
        ...
        //为toolbar添加事件响应
        table.on('tool(accuseAdviceListFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
    ...
    function view(id){
        parent.layer.open({
            ...
            content: "page/approval/accuseAdvice/viewAccuseAdvice.html?id="+id,
    ...
    function approval(id){
        parent.layer.open({
            ...
            content: "page/approval/accuseAdvice/approval.html?id="+id,
            success: function (layero, index) {
            }
        });
    }
    ...
    function revoke(id){
        layer.confirm('确认撤销吗？', function (confirmIndex) {
            ...
            $api.doPost($tool.getContext() + 'approval/revokeAccuseAdvice.do',req,function (data) {
            	if(data.data==1){
