layui.config({
    ...
    function defineTable() {
        ...
        tableIns = table.render({
        	elem: '#manageAuthorityList-data'
           ,toolbar: '#toolbarDemo'
           ,height: 600
           ,url: $tool.getContext() + 'approval/getManageAuthorityList.do' //数据接口
           ,method: 'post'
           ,where: {
           ...
           ,cols: [[ //表头
        	   {type: 'numbers', title: '序号', fixed: 'left'}
          	  ,{field: 'staff', title: '申请人', width: 120}
          	  ...
          	  ,{field: 'reportTime', title: '发起时间', width: 180, templet: "#timeTemplate"}
        ...
        //为toolbar添加事件响应
        table.on('tool(manageAuthorityListFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
    ...
    function view(id){
        parent.layer.open({
            ...
            content: "page/approval/manageAuthority/viewManageAuthority.html?id="+id,
    ...
    function approval(id){
        parent.layer.open({
            ...
            content: "page/approval/manageAuthority/approval.html?id="+id,
            success: function (layero, index) {
            }
        });
    }
    ...
    function revoke(id){
        layer.confirm('确认撤销吗？', function (confirmIndex) {
            ...
            $api.doPost($tool.getContext() + 'approval/revokeManageAuthority.do',req,function (data) {
            	if(data.data==1){
