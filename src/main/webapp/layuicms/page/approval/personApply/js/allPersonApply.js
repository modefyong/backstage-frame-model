layui.config({
    ...
    function defineTable() {
        ...
        tableIns = table.render({
        	elem: '#personApplyList-data'
           ,toolbar: '#toolbarDemo'
           ,height: 600
           ,url: $tool.getContext() + 'approval/getPersonApplyList.do' //数据接口
           ,method: 'post'
           ,where: {
...
//      	  ,{field: 'applyTime', title: '发起时间', width: 180, templet: "<div>{{!d.applyTime? '' :layui.util.toDateString(d.applyTime, \'yyyy-MM-dd\')}}</div>"}
          	  ,{field: 'applyTime', title: '发起时间', width: 180}
//          	  ,{field: 'applyTime', title: '发起时间', width: 180, templet: "#timeTemplate"}
        ...
        //为toolbar添加事件响应
        table.on('tool(personApplyListFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
...
//    	console.log($("#checkState").find("option:selected").val())
        //表格重新加载
        tableIns.reload({
            where:{
            	loginName: loginName,
    ...
    function view(id){
        parent.layer.open({
            ...
            content: "page/approval/personApply/viewPersonApply.html?id="+id,
    ...
    function approval(id){
        parent.layer.open({
            ...
            content: "page/approval/personApply/approval.html?id="+id,
            success: function (layero, index) {
            }
        });
    }
    ...
    function revoke(id){
        layer.confirm('确认撤销吗？', function (confirmIndex) {
            ...
            $api.doPost($tool.getContext() + 'approval/revokePersonApply.do',req,function (data) {
            	if(data.data==1){
