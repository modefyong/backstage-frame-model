layui.config({
    ...
    function init() {
        ...
        $api.doGet($tool.getContext()+'approval/getPersonApply.do',req,function (res) {
            var data = res.data;
            $("[name='approvalHeading']").val(data.applyPerson + '提交的积分-自主申请');
            $("[name='itemKind']").val(data.itemKind);           
            $("[name='detailNum']").val(data.detailNum);
